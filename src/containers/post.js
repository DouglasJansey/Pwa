...
function Post() {
    ...
    const renderImg = ({ image, description }) => <img src={image.url} alt={description} width="80%" />
    ...
    const renderDescriptioin = (description) => <p dangerouslySetInnerHTML={createMarkup(description)} />
        ...
        return (
            <Col span={12} key={`post.${index}`} >
                <article onClick={() => openPost(id)}>
                    ...
                    {image?.url ? renderImg({ image, description }) : renderDescriptioin(description)}
                </article>
            </Col>
        )
    }
    ...
    return (
        <div>
            ...
            <Row gutter={[16, 16]}>
                <Col span={24} md={16}>
                    <p>{datePublished}</p>
                    <h dangerouslySetInnerHTML={createMarkup(title)} />
